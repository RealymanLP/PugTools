    ...
    public class HeightMap
    {
        ...
        public HeightMap(BinaryReader br)
        {
            ...
            if ((headerBitFlag & 8) != 0)
            {
                ...
                Debug.WriteLine("Unknown heightmap floats {0}, {1}, {2}", unkX, unkY, unkZ);
            }
            ...
            // Read center
            // NOTE: unsigned, not signed - verified against the reference reader. Reading this
            // signed would silently produce wrong (negative) heights for any raw value >= 0x8000.
            for (int j = 1; j <= width - 2; j++)
            {
                for (int k = 1; k <= depth - 2; k++)
                {
                    elevation[k, j] = br.ReadUInt16() / 512f;
                }
            }
            ...
            // else
            // {
            //     Debug.WriteLine("ERROR: unknown header byte in heightmap: " + headerBitFlag.ToString());
            // }
        }
