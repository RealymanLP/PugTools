    ...
    private void BackgroundWorker1_DoWork(object sender, DoWorkEventArgs e) {
      ...
      foreach (var kvp in mapAreas) {
        ...
        HashFileInfo hashInfo = new HashFileInfo(
          areaFile.FileInfo.PrimaryHash, areaFile.FileInfo.SecondaryHash, areaFile);

        NodeAsset assetAll = new NodeAsset("/" + id, "/", name, hashInfo);
        newAssetDict.Add("/" + id, assetAll);
      }
...
#pragma warning disable CS1998, CS4014
    private async Task PreviewAREA(HashFileInfo info, ulong areaId) {
      ...
      int totalInstances = 0, matchedAssets = 0, missingGr2Files = 0, loadedModels = 0, failedGr2Loads = 0;
        ...
        SetStatusLabel(string.Format(
          "Rooms:{0} GR2Assets:{1} Instanzen:{2} zugeordnet:{3} geladen:{4} GR2fehlt:{5} GR2Fehler:{6} | {7}",
