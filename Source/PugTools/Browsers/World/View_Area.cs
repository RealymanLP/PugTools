...
namespace PugTools {
  class View_AREA : D3DPanelApp {
    ...
    private readonly List<GR2_Mesh_Vertex_Index> indicies = new List<GR2_Mesh_Vertex_Index>();
          ...
          foreach (KeyValuePair<ulong, List<AssetInstance>> kvp in room.InstancesByAssetId) {
            foreach (AssetInstance instance in kvp.Value) {
              ...
              if (instance.hasHeightMap) {
                Util.ReleaseCom(ref instance.VBO);
                Util.ReleaseCom(ref instance.IBO);
              }
              ...
              if (models.Keys.Contains(instance.assetID)) {
                ...
                if (model.attachedModels.Count > 0) {
                  foreach (var attach in model.attachedModels) {
    ...
    public override void OnResize() {
      base.OnResize();
      camera.SetLens(0.25f * SlimDXNet.MathF.PI, AspectRatio, 0.001f, 1000.0f);
    }
                ...
                foreach (var attachModel in model.attachedModels) {
                  ...
                  foreach (var attachMesh in attachModel.meshes) {
                    ...
                    foreach (GR2_Mesh_Piece attachPiece in attachMesh.meshPieces) {
                      ImmediateContext.InputAssembler.SetVertexBuffers(0, new VertexBufferBinding(attachMesh.vertBuffer, PosNormalTexTan.Stride, 0));
                      ImmediateContext.InputAssembler.SetIndexBuffer(attachMesh.idxBuffer, Format.R16_UInt, 0);
    ...
    protected override void OnMouseMove(object sender, MouseEventArgs e) {
      if (e.Button == MouseButtons.Left) {
        var dy = SlimDXNet.MathF.ToRadians(0.4f * (e.Y - lastMousePos.Y));
        var dx = -SlimDXNet.MathF.ToRadians(0.4f * (e.X - lastMousePos.X));
        camera.Pitch(-dy);
        camera.Yaw(dx);
      }
          ...
          foreach (AssetInstance instance in kvp.Value) {
            ...
            if (models.Keys.Contains(instance.assetID)) {
              ...
              foreach (var mesh in model.meshes) {
                ...
                mesh.vertBuffer = new Buffer(Device, new DataStream(vertices.ToArray(), false, false), vbd);


                ushort[] indexArray = mesh.meshVertIndex.Select(GR2_Mesh_Vertex_Index => GR2_Mesh_Vertex_Index.index).ToArray();
                var ibd = new BufferDescription(sizeof(ushort) * indexArray.Length, ResourceUsage.Immutable, BindFlags.IndexBuffer, CpuAccessFlags.None, ResourceOptionFlags.None, 0);
              ...
              if (model.attachedModels.Count > 0) {
                foreach (var attachModel in model.attachedModels) {
                  foreach (var attachMesh in attachModel.meshes) {
                    ...
                    var vbd = new BufferDescription(PosNormalTexTan.Stride * vertices.Count, ResourceUsage.Immutable, BindFlags.VertexBuffer, CpuAccessFlags.None, ResourceOptionFlags.None, 0);
                    attachMesh.vertBuffer = new Buffer(Device, new DataStream(vertices.ToArray(), false, false), vbd);


                    ushort[] indexArray = attachMesh.meshVertIndex.Select(GR2_Mesh_Vertex_Index => GR2_Mesh_Vertex_Index.index).ToArray();
                    var ibd = new BufferDescription(sizeof(ushort) * indexArray.Length, ResourceUsage.Immutable, BindFlags.IndexBuffer, CpuAccessFlags.None, ResourceOptionFlags.None, 0);
                    attachMesh.idxBuffer = new Buffer(Device, new DataStream(indexArray, false, false), ibd);
                  }
                }
              }
            }
