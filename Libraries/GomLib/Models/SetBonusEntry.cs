  ...
  public class SetBonusEntry : PseudoGameObject, IEquatable<SetBonusEntry> {
    ...
    public List<Item> Sources {
      get {
        if (Sources_ == null) {
          ...
          Sources_ = SourcesIds.Select(x => Dom.ItemLoader.Load(x)).ToList();
        }
        return Sources_;
      }
    }
    ...
    public Dictionary<long, Ability> BonusAbilityByNum {
      get {
        if (BonusAbilityByNum_ == null) {
          ...
          BonusAbilityByNum_ = BonusAbilityIdsByNum.ToDictionary(x => x.Key, x => Dom.AbilityLoader.Load(x.Value));
        }
    ...
    public Dictionary<long, Dictionary<string, string>> LocalizedBonusDescriptions {
      get {
        ...
        return BonusAbilityIdsByNum.ToDictionary(x => x.Key, x => (Dom.AbilityLoader.Load(x.Value) ?? new Ability()).ParsedLocalizedDescription);
      }
    }
    #endregion
    ...
    public bool Equals(SetBonusEntry entry) {
      ...
      if (Sources.Count != entry.Sources.Count)
        return false;
      if (LocalizedName != null && entry.LocalizedName != null) {
        if (!LocalizedName.SequenceEqual(entry.LocalizedName)) {
          return false;
        }
      }
