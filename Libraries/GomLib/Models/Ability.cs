...
namespace GomLib.Models {
  public class Ability : GameObject, IEquatable<Ability> {
    ...
    public Dictionary<string, string> ParsedLocalizedDescription {
      get {
        if (ParsedLocalizedDescription_ == null && LocalizedDescription != null) {
          ParsedLocalizedDescription_ = new Dictionary<string, string>();
          foreach (var desc in LocalizedDescription) {
            ParsedLocalizedDescription_.Add(desc.Key, ParseDescription(desc.Value));
          }
        }
        return ParsedLocalizedDescription_;
      }
    }
      ...
      for (var i = 0; i < abl.DescriptionTokens.Count; i++) {
        ...
        if (end > 5) {
          try {
            ...
            var partialToken = fullToken[4..^3];
        ...
        //console.log(type);
        while (desc.IndexOf(fullToken) != -1) { //sometimes there's multiple instance of the same token.
    ...
    [JsonIgnore]
    internal List<string> Effects_ { get; set; }
    public List<string> EffectsB62 {
      get {
        if (Effects_ == null && EffectIds != null) {
          Effects_ = new List<string>();
          foreach (var uId in EffectIds) {
            Effects_.Add(uId.ToMaskedBase62());
          }
        }
        return Effects_;
      }
    ...
    public Dictionary<int, Dictionary<string, object>> ModDescriptionTokens {
      get {
        ...
        if (ModDescriptionTokens_ == null) {
          ModDescriptionTokens_ = new Dictionary<int, Dictionary<string, object>>();
          foreach (var token in DescriptionTokens) {
            ModDescriptionTokens_.Add(token.Key, token.Value.ToDictionary(x => x.Key.Replace("ablDescriptionToken", "")
    ...
    internal List<Dictionary<string, float>> AbsorbParams_ { get; set; }
    public List<Dictionary<string, float>> AbsorbParams {
      get {
        if (AbsorbParams_ == null) {
          AbsorbParams_ = new List<Dictionary<string, float>>();
        }

        return AbsorbParams_;
      }
    ...
    public string HashedIcon {
      get {
        ...
        var fileId = TorArchive.FileId.FromFilePath(string.Format("/resources/gfx/icons/{0}.dds", icon));
    ...
    public override int GetHashCode()  //should be fixed.
    {
      ...
      if (DescriptionTokens != null) {
        foreach (var x in DescriptionTokens) {
          ...
          if (x.Value.TryGetValue("ablDescriptionTokenType", out object typeObj)) {
            type = (string)typeObj;
          }
          ...
          foreach (var val in x.Value) {
            ...
            if (val.Key == "ablParsedDescriptionToken" &&
              ...
              if (val.Value is string valueStr) {
                hash ^= valueStr.GetHashCode();
              } else {
                if (val.Value is long @int) {
                  hash ^= @int.GetHashCode();
                } else if (val.Value is float single) {
    ...
    public override bool Equals(object obj) {
      ...
      if (obj is not Ability abl) return false;
    ...
    public override string ToString(bool verbose) {
      ...
      if (ChannelingTime != 0) { txtFile.Append("  Channeling Time: " + ChannelingTime + n); } else {
        if (CastingTime != 0) { txtFile.Append("  Casting Time: " + CastingTime + n); } else { txtFile.Append("  Casting Time: Instant" + n); }
      }
      ...
      if (DescriptionTokens != null && DescriptionTokens.Count > 0) {
        ...
        foreach (KeyValuePair<int, Dictionary<string, object>> kvp in DescriptionTokens) {
          ...
          if (kvp.Value.TryGetValue("ablDescriptionTokenType", out object tokenTypeObj)) {
    ...
    public static string GenerateTokenString(string value) {
