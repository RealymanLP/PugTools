...
namespace GomLib.Models {
  public class AssetInstance : PseudoGameObject, IEquatable<AssetInstance> {
      ...
      get {
        if (AssetFqn_ == null) {
          ...
          if (parsedAssetString.StartsWith("\\"))
            parsedAssetString = parsedAssetString[1..];
          ...
          switch (assType) {
            case "ser":
              parsedAssetString = parsedAssetString[7..^4];
              break;
            case "spn":
            case "enc":
              if (parsedAssetString.Contains(".spn_")) {
                SpawnType = parsedAssetString[(parsedAssetString.IndexOf(".spn_") + 5)..];
              }
              parsedAssetString = parsedAssetString.Replace(".spn_" + SpawnType, "");
              if (parsedAssetString.EndsWith(".enc"))
                parsedAssetString = parsedAssetString[0..^4];
              break;
            default:
              break;
          }
          parsedAssetString = parsedAssetString.Replace('\\', '.');
          var obj = Room.Dom.GetObject(parsedAssetString);
          if (obj == null) {
    ...
    public override bool Equals(object obj) {
      ...
      if (obj is not Area itm) return false;
  ...
  public class Vec3 : IEquatable<Vec3> {
    ...
    public override bool Equals(object obj) {
      ...
      if (obj is not Vec3 itm) return false;
