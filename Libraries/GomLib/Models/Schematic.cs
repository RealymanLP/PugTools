...
namespace GomLib.Models {
  public class Schematic : GameObject, IEquatable<Schematic> {
    ...
    public Item Item {
      get {
        if (Item_ == null)
          Item_ = Dom_.ItemLoader.Load(ItemId);
        return Item_;
      }
      set {
        Item_ = value;
    ...
    public int MissionResultDescriptionId { get; set; }
    public string MissionResultDescription { get; set; }
    public Dictionary<string, string> LocalizedMissionDescription { get; set; }
    public Dictionary<string, string> LocalizedMissionResultDescription { get; set; }
    ...
    public override bool Equals(object obj) {
      ...
      if (obj is not Schematic sch) return false;
    ...
    public override XElement ToXElement(bool verbose) {
      ...
      if (Name == "") {
        if (ItemId != 0) {
          var crafted = new GameObject().ToXElement(ItemId, Dom_, verbose);
        ...
        if (Materials != null) {
          foreach (var kvp in Materials) {
            mats.Add(RequiredMatToXElement(Dom_, kvp.Value, kvp.Key, verbose));
          }
          schem.Add(mats);
        }
        XElement resmats = new XElement("ResearchReturnedMaterials");
        if (Research1 != null)
          resmats.Add(RequiredMatToXElement(Dom_, ResearchQuantity1, Research1.Id, verbose));
        if (Research2 != null)
          resmats.Add(RequiredMatToXElement(Dom_, ResearchQuantity2, Research2.Id, verbose));
        if (Research3 != null)
          resmats.Add(RequiredMatToXElement(Dom_, ResearchQuantity3, Research3.Id, verbose));
        schem.Add(resmats);
      } else {
    ...
    private static XElement RequiredMatToXElement(DataObjectModel dom, int i, ulong id, bool verbose) {
