...
namespace GomLib.Models {
  public class Collection : PseudoGameObject, IEquatable<Collection> {
    ...
    public List<Ability> AbilityList {
      get {
        ...
        foreach (var ability in AbilityIdsList) {
          LoadedAbilityList.Add(Dom.AbilityLoader.Load(ability));
        }
        return LoadedAbilityList;
      }
    }
    ...
    public List<Item> ItemList {
      get {
        ...
        foreach (var item in ItemIdsList) {
          LoadedItemList.Add(Dom.ItemLoader.Load(item));
        }
        return LoadedItemList;
      }
    }
    ...
    public override bool Equals(object obj) {
      ...
      if (obj is not Collection col) return false;
    ...
    public override XElement ToXElement(bool verbose) {
      ...
      if (Unknowntext != "") { collectionXE.Add(new XElement("Unknown", Unknowntext)); } else { collectionXE.Add(new XElement("Unknown")); }
      ...
      foreach (var altUnlock in AlternateUnlocksMap) {
        XElement alternate = new GameObject().ToXElement(altUnlock.Key, Dom, false);
        foreach (var baseAlternate in altUnlock.Value) {
          alternate.Add(new XElement("AlternateUnlockFor", new GameObject().ToXElement(baseAlternate, Dom, false)));
        }
