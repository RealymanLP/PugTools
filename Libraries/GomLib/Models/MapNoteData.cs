  ...
  public class MapNoteData : GameObject {
    ...
    public MapNoteData Load(XElement node) {
      foreach (var f in node.Elements()) {
        ...
        switch (attrib.Value) {
          case "mpnPosition":
            var temp_vec = f.Value[1..^1];
            var split = temp_vec.Split(',');
            if (split.Length == 3) {
              _ = float.TryParse(split[0], out float x);
              PositionX = x;
              _ = float.TryParse(split[1], out float y);
              PositionY = y;
              _ = float.TryParse(split[2], out float z);
              PositionZ = z;
            }
            break;
          case "mpnRotation":
            ...
            if (rotsplit.Length == 3) {
              _ = float.TryParse(rotsplit[1], out float r);
              Rotation = r;
            }
            break;
          case "mpnMapTags":
            ...
            for (var m = 0; m < f.Elements().Count(); m += 2) {
              ...
              _ = bool.TryParse(e.Value, out bool mtb);
          ...
          case "ShowGhosted":
            bool g;
            _ = bool.TryParse(f.Value, out g);
            ShowGhosted = g;
            break;
          case "ShowOffmapArrow":
            bool o;
            _ = bool.TryParse(f.Value, out o);
            ShowOffmapArrow = o;
            break;
          case "ShowQuestBreadcrumbOnly":
            bool b;
            _ = bool.TryParse(f.Value, out b);
            ShowQuestBreadcrumbOnly = b;
            break;
          case "DoNotBreadcrumb":
            bool d;
            _ = bool.TryParse(f.Value, out d);
            DoNotBreadcrumb = d;
            break;
          case "mpnIgnoreFoW":
            bool w;
            _ = bool.TryParse(f.Value, out w);
            IgnoreFoW = w;
            break;
          case "mpnTemplateFQN":
