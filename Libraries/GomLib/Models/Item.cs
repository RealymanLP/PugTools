  ...
  public class Item : GameObject, IEquatable<Item> {
    public override string ToString() {
      return string.Format("{0:000000} {1}: [{2}] {3}",
          Id, LocalizedName[GomLib.StringTable.SelectedLocalization] ?? "", ItemLevel, StatModifiers);
    }
    ...
    public string HashedImperialIcon {
      get {
        ...
        var fileId = TorArchive.FileId.FromFilePath(string.Format("/resources/gfx/icons/{0}.dds", ImperialIcon));
        return string.Format("{0}_{1}", fileId.Ph, fileId.Sh);
      }
    }
    public string HashedRepublicIcon {
      get {
        if (string.IsNullOrEmpty(RepublicIcon)) return "";
        var fileId = TorArchive.FileId.FromFilePath(string.Format("/resources/gfx/icons/{0}.dds", RepublicIcon));
        return string.Format("{0}_{1}", fileId.Ph, fileId.Sh);
      }
    }
    [JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
    ...
    [JsonConverter(typeof(LongConverter))]
    public long EquipSetBonus { get; set; }
    ...
    private static List<long> _ArmorSpecIds = new List<long> { -8622546409652942944, 589686270506543030, 5763611092890301551 };
    public static List<long> ArmorSpecIds {
      get => _ArmorSpecIds;
      set => _ArmorSpecIds = value;
    }
    private static List<ArmorSpec> _ArmorSpecs = new List<ArmorSpec>();
    public static List<ArmorSpec> ArmorSpecs {
      get => _ArmorSpecs;
      set => _ArmorSpecs = value;
    }
    ...
    public override bool Equals(object obj) {
      ...
      if (obj is not Item itm) return false;
    ...
    public override XElement ToXElement(bool verbose) {
      ...
      if (UseAbilityId == 0) { item.Add(new XElement("UseAbility")); } else { item.Add(new XElement("UseAbility", UseAbility.ToXElement(verbose))); }
      if (EquipAbilityId == 0) { item.Add(new XElement("EquipAbility")); } else { item.Add(new XElement("EquipAbility", EquipAbility.ToXElement(verbose))); }
      ...
      if (verbose) {
        ...
        if (reqclasses != null) { reqclasses = reqclasses[0..^2]; }
