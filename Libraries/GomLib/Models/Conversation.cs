...
namespace GomLib.Models {
  public class Conversation : GameObject, IEquatable<Conversation> {
    ...
    public Dictionary<string, string> SpeakersB62Ids {
      get {
        if (_SpeakersB62Ids == null) {
          ...
          _SpeakersB62Ids = SpeakersIds.ToDictionary(x => x.ToMaskedBase62(), x => (Dom_.GetObject(x) ?? new GomObject()).Name);
        }
    ...
    public Dictionary<ulong, GameObject> Speakers {
      get {
        ...
        for (int i = 0; i < SpeakersIds.Count; i++) {
          speaks.Add(SpeakersIds[i], Dom_.ConversationLoader.LoadSpeaker(SpeakersIds[i]));
        }
    ...
    public override bool Equals(object obj) {
      ...
      if (obj is not Conversation cnv) return false;
