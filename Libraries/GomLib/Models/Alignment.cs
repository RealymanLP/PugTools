...
namespace GomLib.Models {
  public class AlignmentData {
    ...
    public AlignmentTier ToLightTier(int id) {
      ...
      if (!AlignmentTierLookup["Light"].ContainsKey(id)) { return null; } else { return AlignmentTierLookup["Light"][id]; }
    }
    public AlignmentTier ToDarkTier(int id) {
      if (AlignmentTierLookup == null) LoadData();

      if (!AlignmentTierLookup["Dark"].ContainsKey(id)) { return null; } else { return AlignmentTierLookup["Dark"][id]; }
    }
    public AlignmentTier ToNeutralTier(int id) {
      if (AlignmentTierLookup == null) LoadData();
      if (id > 0) throw new NotImplementedException();
      if (!AlignmentTierLookup["Neutral"].ContainsKey(id)) { return null; } else { return AlignmentTierLookup["Neutral"][id]; }
    }
    ...
    private AlignmentTier LoadAlignmentTier(GomObjectData gom, int i) {
      ...
      socTier.Name = socTier.LocalizedName[GomLib.StringTable.SelectedLocalization];
