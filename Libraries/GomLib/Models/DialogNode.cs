...
namespace GomLib.Models {
  public class DialogNode : IEquatable<DialogNode> {
    ...
    internal Dictionary<Npc, KeyValuePair<int, string>> AffectionRewards_ { get; set; }
    [JsonIgnore]
    public Dictionary<Npc, KeyValuePair<int, string>> AffectionRewards {
      get {
        if (AffectionRewards_ == null) {
          AffectionRewards_ = new Dictionary<Npc, KeyValuePair<int, string>>();
          foreach (var kvp in AffectionRewardEvents) {
            Npc companion = Conversation.Dom_.ConversationLoader.CompanionBySimpleNameId(kvp.Key);
            if (companion != null)
              AffectionRewards_[companion] = kvp.Value;
          }
        }
        return AffectionRewards_;
      }
    }
    [JsonIgnore]
    ...
    public override bool Equals(object obj) {
      ...
      if (obj is not DialogNode dln) return false;
