  ...
  public class SchematicLoader {
    ...
    public Schematic Load(Schematic schem, GomObject obj) {
      ...
      /*if (itemId > 0)
      {
          ...
          {
              Debug.WriteLine("Schematic references non-existant item: " + schem.Fqn);
          }
      }*/
      ...
      if (string.IsNullOrEmpty(schem.Name) && (schem.Item != null)) {
        //schem.Name = schem.Item.LocalizedName[GomLib.StringTable.SelectedLocalization];
        schem.LocalizedName = schem.Item.LocalizedName;
      }
      schem.LocalizedName = Normalize.Dictionary(schem.LocalizedName, schem.Fqn);
      schem.Name = schem.LocalizedName[GomLib.StringTable.SelectedLocalization];
      ...
      if (schem.MissionDescriptionId > 0) {
        ...
        if (missionStrTable.GetText((schem.MissionDescriptionId + 1) + strOffset, schem.Fqn).Contains("&lt;&lt;1&gt;&gt;") || missionStrTable.GetText((schem.MissionDescriptionId + 1) + strOffset, schem.Fqn).Contains("<<1>>")) {
          schem.MissionResultDescriptionId = (schem.MissionDescriptionId + 1);
          schem.MissionResultDescription = missionStrTable.GetText(schem.MissionResultDescriptionId + strOffset, schem.Fqn);
          schem.LocalizedMissionResultDescription = missionStrTable.GetLocalizedText(schem.MissionResultDescriptionId + strOffset, schem.Fqn);
        }
      }
      ...
      if (schem.MissionDescriptionId != 0) {
        ...
        schem.Category = schem.LocalizedCategory[GomLib.StringTable.SelectedLocalization];
      ...
      _dom.ItemLoader.schematicLookupMap.TryGetValue(schem.Id, out List<ulong> pIds);
      schem.LearnedIds = pIds ?? new List<ulong>(); //null coalesce so we don't have to account for it later.

      if (idMap.Values.Where(s => s.Id == schem.Id).Any()) {
