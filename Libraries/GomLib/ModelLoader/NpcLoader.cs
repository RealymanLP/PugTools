        ...
        foreach (var toughness in Enum.GetValues(typeof(Toughness)).Cast<Toughness>()) {
          switch (toughness) {
            ...
            case Toughness.Boss4:
              ...
              break;
            case Toughness.Companion:
              ToughnessLocalizationMap.Add(toughness, portTable.GetLocalizedText(946292964458519, "str.gui.protraitbar"));
              break;
            case Toughness.None:
            default:
      ...
      if (npcVisualDataList != null) {
        ...
        foreach (var visualDataObject in npcVisualDataList) {
          ...
          NpcVisualData nvd = new NpcVisualData(_dom)
            ...
            MeleeOffWepId = visualData.ValueOrDefault<ulong>("npcTemplateVisualDataMeleeOffWeapon", 0),
            //if (nvd.MeleeOffWepId != 0) nvd.MeleeOffWep = _dom.itemLoader.Load(nvd.MeleeOffWepId);
            RangedWepId = visualData.ValueOrDefault<ulong>("npcTemplateVisualDataRangedWeapon", 0),
            //if (nvd.RangedWepId != 0) nvd.RangedWep = _dom.itemLoader.Load(nvd.RangedWepId);
            RangedOffWepId = visualData.ValueOrDefault<ulong>("npcTemplateVisualDataRangedOffWeapon", 0),
            //if (nvd.RangedOffWepId != 0) nvd.RangedOffWep = _dom.itemLoader.Load(nvd.RangedOffWepId);
            AppearanceId = visualData.ValueOrDefault<ulong>("npcTemplateVisualDataAppearance", 0)
          };
