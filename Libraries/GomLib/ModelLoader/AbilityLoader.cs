...
namespace GomLib.ModelLoader {
  public class AbilityLoader : IModelLoader {
    ...
    public Ability Load(GameObject obj, GomObject gom) {
      ...
      _dom.Assets.Icons.Add(abl.Icon);
      ...
      abl.Name = abl.LocalizedName[GomLib.StringTable.SelectedLocalization];
          ...
          foreach (var key in eff.Data.Dictionary.Keys) {
            if (key.StartsWith("4611")) {
              if (eff.Data.Get<object>(key).GetType() == new List<object>().GetType()) {
                if (((List<object>)eff.Data.Get<object>(key)).Any()) {
      ...
      if (sharedCooldowns != null) {
        ...
        if (sharedCooldowns.Count > 1) {
          //Debug.WriteLine("{0} has more than 1 shared cooldown? How does this work.", abl.Fqn);
        }
...
tempString = start + middle + end;
    ...
    public Dictionary<string, object> LoadDescriptionToken(GomObjectData tokDesc, List<object> abilityEffectList) {
      ...
      switch (tokType) {
        ...
        case "ablDescriptionTokenTypeDuration":
          ...
          tokenInfo.Add("ablParsedDescriptionToken", float.Parse(duration[(duration.IndexOf(",") + 1)..]));
          break;
    ...
    private static List<Dictionary<string, string>> LoadActionParams(GomObjectData action) {
