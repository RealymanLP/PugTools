...
namespace GomLib.ModelLoader {
  public class AreaLoader {
    ...
    public Area Load(Area area, GomObjectData obj) {
      ...
      var mapNoteObj = _dom.Assets.FindFile(mapNotePath);
      if (mapNoteObj != null) {
    ...
    private static void LoadMapNotes(Area area, TorArchive.File file) {
      string xml = "";
      ...
      for (int i = 0; i < count; i += 2) {
        ...
        _ = ulong.TryParse(key, out ulong k);
        var value = elements.ElementAt(i + 1);
        var node = value.Element("node");
        if (node != null) {
          MapNoteData mpn = new MapNoteData(area.Dom).Load(node);
          mapNotes.Add(k, mpn);
        } else {
          continue;
        }
    ...
    private void LoadMapdata(Area area, GomObject obj) {
      ...
      if (mapPages != null) {
        ...
        foreach (GomObjectData mapPage in mapPages) {
          ...
          page.HasImage = _dom.Assets.HasFile(mapImagePath);
          ...
          _dom.Assets.Icons.AddMap(area.AreaId, page.MapName);
