    ...
    public Decoration Load(GameObject obj, GomObject gom) {
      ...
      foreach (var key in keys.Skip(3)) //Skip the first three keys as they are internal ones
      {
        if (!fields.Contains(key)) {
          //Debug.WriteLine(String.Join(", ", key, dec.Fqn));
      ...
      dec.Name = dec.LocalizedName[GomLib.StringTable.SelectedLocalization];
      ...
      if (data != null) {
        //Debug.WriteLine(String.Format("Field {0} set to: {1} type: {2}, {3}", "4611686351126997007", data.ToString(), data.GetType().ToString(), gom.Name));
      }
      ...
      if (dec.UniquePerLegacy) {
        //Debug.WriteLine(String.Format("Field {0} set to: {1}, {2}", "decUniquePerLegacy", dec.UniquePerLegacy, gom.Name));
      }
      ...
      if (decCats != null) {
        ...
        tableNameLookup.TryGetValue(dec.CategoryId, out object catTab);
      ...
      if (decSubCats != null) {
        ...
        tableNameLookup.TryGetValue(dec.SubCategoryId, out object catTab);
